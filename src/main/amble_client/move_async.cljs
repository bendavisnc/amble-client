(ns amble-client.move-async
  "Abstracts websockets interaction for receiving move async updates from server.
   Responsible for adding to the move index channel."
...
(go-loop [game-id (:game-id (deref (async/<! app-atom-chan)))
  ...
  (let [move-from-server (async/<! move-source-chan)]
    ...
    (async/>! move-index-chan move-from-server))
  (recur game-id
         websockets-connection-chan
         move-source-chan))

(defmethod ig/init-key :amble/move-async [_ {:keys [app-atom, move-index-chan, app-ready-chan]}]
  (async/pipe amble-client.move-async/move-index-chan move-index-chan)
  (go
